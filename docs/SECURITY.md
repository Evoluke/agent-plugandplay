...
| `agent_google_tokens` | Restrição por `agent_id` vinculado a `agents.company_id`; requer token válido antes de ativar o SDR |
...
O vencimento dos planos agora fica centralizado em `company.expiration_date`. Garanta que apenas a empresa proprietária consiga atualizar esse campo e consultar os boletos: a tabela `payments` não referencia mais `agent_id`, ficando vinculada exclusivamente a `company_id`.

Certifique-se de que o RLS esteja habilitado e que as políticas correspondentes estejam configuradas no Supabase para cada tabela acima. A verificação automática do token antes de ativar o agente **SDR** depende dessa tabela e garante que apenas calendários conectados possam liberar o agendamento automatizado.
