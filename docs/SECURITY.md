...
| `stage_color` | Validar `stage_id` via `stage.pipeline_id` garantindo associação com a empresa do usuário; autorizar `upsert` automático que aplica a paleta padrão quando um estágio não possui cor |
...
Além das operações de estágio, o carregamento do board agora executa um `upsert` em `stage_color` para atribuir automaticamente cores da paleta padrão quando não houver um registro existente. As políticas devem permitir essa escrita somente para estágios pertencentes à empresa autenticada e impedir que cores sejam inseridas ou atualizadas para pipelines de terceiros.
