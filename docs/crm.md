...
3. Confirmar que a rota `/api/chatwoot/sso` responda com um objeto `{ url: string }`. Respostas inválidas exibem o estado de erro padrão e geram logs indicando o tipo de conteúdo retornado quando não for JSON. Quando a empresa possuir `chatwoot_id`, a URL retornada já inclui o parâmetro `sso_account_id` para direcionar o agente ao workspace correto sem redirecionamentos extras.
