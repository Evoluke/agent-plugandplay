...
- As cores de fundo dos estágios são configuradas via tabela `stage_color`; selecione valores alinhados à paleta e mantenha contrastes mínimos de 4,5:1 com o texto.
- O front-end converte cada cor em um gradiente suave aplicado ao contorno e ao fundo da coluna, preservando legibilidade com sobreposição translúcida.
- Caso nenhum valor seja encontrado em `stage_color`, o sistema aplica automaticamente uma cor da paleta padrão (`DEFAULT_STAGE_COLORS`) para evitar colunas sem identidade visual; personalize o registro correspondente se desejar outra tonalidade.
